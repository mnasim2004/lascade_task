...
async def process_image(file: UploadFile = File(...)):
    ...
    # Create a PNG file from the rendered mesh
    png_filename = "output.png"
    plotter.add_mesh(combined_mesh, color='cyan')  # You can change the color as needed
    plotter.view_xy()  # Set the view to top-down
    plotter.screenshot(png_filename)  # Take a screenshot

    # Clean up plotter
    plotter.close()

    # Return only the PNG file as a response
    return FileResponse(png_filename, media_type='image/png', filename=png_filename)
